...
module Emojidex
  ...
  class Converter
    ...
    @@format_types = Emojidex::Converters.Available
    ...
    def convert(emoji, source_path)
      if File.directory?("#{source_path}/#{emoji.code}")
        png = Emojidex::Converters::PNG.new
        png.convert_to_apng(emoji.code, @sizes, source_path)
      else
        svg = Emojidex::Converters::SVG.new
        svg.convert_to_png(emoji.code, @sizes, source_path)
      end
    end
  end
end
